...
A social platform for sharing real-life experiences through multi-step routes with seamless map integration and vertical feed.

![Reune Brand](https://img.shields.io/badge/Color-%231ABB6C-brightgreen) ![Font](https://img.shields.io/badge/Font-Baloo%20Tamma-blue)
...
### Centralized Friend System
All friend relationships are managed through a central `friendStatuses` Map in App.tsx:
- Single source of truth for all friend data
- Consistent across all components (Profile, FriendsList, UserProfile, etc.)
- Real-time updates propagate throughout the app

### Bidirectional Continuity
Home feed and map view share critical state:
- `selectedRouteId`: Currently selected route
- `selectedRouteStepIndex`: Current step in the route
- `selectedPlaceLocation`: Place to highlight on map
- `selectedTemplate`: Shared template selection

## Installation & Setup

```bash
# This is a Figma Make project
# To run locally, you would need:

# 1. Clone the repository
git clone https://github.com/ReuneApp/reune_figma.git
cd reune_figma

# 2. Install dependencies (in a typical React setup)
npm install
